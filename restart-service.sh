#!/bin/bash

# Usage: ./restart-service.sh <service-name>
# Example: ./restart-service.sh auth-service

SERVICE=$1
GREEN='\033[0;32m'
RED='\033[0;31m'
NC='\033[0m'
ROOT_DIR="$(cd "$(dirname "$0")" && pwd)"

if [ -z "$SERVICE" ]; then
    echo "Usage: $0 <service-name>"
    echo "Available services: registry, auth, risk, screening, document, viewer, gateway"
    exit 1
fi

case $SERVICE in
    "registry"|"service-registry")
        MODULE="service-registry"; PORT=8761; LOG="registry.log"; NAME="Service Registry" ;;
    "auth"|"auth-service")
        MODULE="auth-service";     PORT=8084; LOG="auth.log";      NAME="Auth Service" ;;
    "risk"|"risk-service")
        MODULE="risk-service";     PORT=8081; LOG="risk.log";      NAME="Risk Service" ;;
    "screening"|"screening-service")
        MODULE="screening-service"; PORT=8082; LOG="screening.log"; NAME="Screening Service" ;;
    "document"|"document-service")
        MODULE="document-service"; PORT=8085; LOG="document.log";  NAME="Document Service" ;;
    "viewer"|"viewer-service")
        MODULE="viewer";           PORT=8083; LOG="viewer.log";    NAME="Viewer Service" ;;
    "gateway"|"api-gateway")
        MODULE="api-gateway";      PORT=8080; LOG="gateway.log";   NAME="API Gateway" ;;
    *)
        echo -e "${RED}Unknown service: $SERVICE${NC}"
        echo "Available services: registry, auth, risk, screening, document, viewer, gateway"
        exit 1 ;;
esac

echo -e "Restarting ${GREEN}$NAME${NC}..."

# Stop
PID=$(lsof -t -i:$PORT -sTCP:LISTEN)
if [ -n "$PID" ]; then
    echo "Stopping existing process (PID $PID) on port $PORT..."
    kill $PID
    while lsof -i:$PORT > /dev/null; do sleep 1; done
    echo "Process stopped."
else
    echo "No running process found on port $PORT."
fi

# Start
echo "Starting $NAME..."
nohup "$ROOT_DIR/gradlew" -p "$ROOT_DIR" :${MODULE}:bootRun > "$ROOT_DIR/$LOG" 2>&1 &

echo "Waiting for $NAME to be ready on port $PORT..."
while ! nc -z localhost $PORT; do sleep 1; done

echo -e "${GREEN}$NAME restarted successfully!${NC}"
echo "Logs available in $LOG"
