...
public class RiskAssessmentService {
            ...
            if (response != null && response.clientRiskRatingResponse() != null) {
                for (RiskDTOs.ClientRiskRatingResponseItem item : response.clientRiskRatingResponse()) {
                    try {
                        ...
                        Integer riskScore = item.overallRiskAssessment() != null ? item.overallRiskAssessment().overallRiskScore() : 0;
                        String riskLevel = item.overallRiskAssessment() != null ? item.overallRiskAssessment().overallRiskLevel() : "LOW";

                        RiskAssessment assessment = new RiskAssessment(
                                null, // assessmentID will be generated by DB
                                null, // logID (not set for manual risk)
                                item.recordID(),
                                riskScore,
                                riskLevel, // initialRiskLevel
                                riskLevel, // overallRiskLevel
                                "MANUAL", // typeOfLogicApplied
                                null, // smeRiskAssessment
                                LocalDateTime.now() // createdAt
                        );
                        riskAssessmentRepository.saveAssessment(assessment);

                        List<Case> cases = caseRepository.findByClientId(clientId);
                        for (Case c : cases) {
                            eventService.logEvent(c.caseID(), "RISK_CHANGED",
                                    "Risk Score updated via Risk Service to " + riskScore,
                                    "SYSTEM");
                        }
                    } catch (Exception e) {
                        // ignore
    ...
    public java.util.List<RiskAssessment> getAssessmentsByRecordId(String recordId) {
        // recordId here is the clientId; return all assessments for this client,
        // covering both manual ("{clientId}") and pre-check ("PRECHECK-{clientId}") RecordID patterns
        return riskAssessmentRepository.findAllByClientId(recordId);
    }
    ...
    public com.venus.kyc.viewer.risk.RiskAssessment getLatestRiskAssessment(Long clientId) {
        // Get the latest risk assessment for this client, considering both manual ("{clientId}")
        // and pre-check ("PRECHECK-{clientId}") record ID patterns
        return riskAssessmentRepository.findLatestByClientId(clientId);
    }
